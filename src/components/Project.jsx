import React from 'react'
import { PROJECTS } from '../constants'
import { FaLink } from 'react-icons/fa'
import { IoArrowForwardOutline } from 'react-icons/io5'
import { motion } from "motion/react"


const Project = () => {
  return (
    <div className='border-b border-neutral-900 pb-4'>
  <motion.h1 whileInView={{opacity:1, y:0}}
            initial={{opacity:0, y:-100}}
            transition={{duration:0.5}} className='my-20 text-center text-4xl'>Projects</motion.h1>
  <div>
    {PROJECTS.map((projects, index)=>(
        <div key={index} className='mb-8 flex flex-wrap lg:justify-center'>
            <motion.div whileInView={{opacity:1, x:0}}
            initial={{opacity:0, x:-100}}
            transition={{duration:1}} className='w-full lg:w-1/4 '>
              
              <img src={projects.image} width={150} height={150} className='mb-8 rounded' alt={projects.title} />  
             </motion.div>
          <motion.div whileInView={{opacity:1, x:0}}
            initial={{opacity:0, x:100}}
            transition={{duration:1}} 
            className='w-full  max-w-xl lg:w-3/4'>
            <a href={projects.link} target="_blank" rel="noopener noreferrer"> 
                <h6 className='mb-2 font-semibold flex items-center justify-between '> <span>{projects.title}</span> <span className='flex justify-center items-center pb-10 gap-2 text-1xl text-cyan-500  '> <IoArrowForwardOutline />Link <FaLink /></span> </h6>  </a>
         
          <p className='mb-4  text-neutral-400'>{projects.description}</p >
          {projects.technologies.map((tech, index)=>(
            <span key={index} className='mr-2 rounded bg-neutral-900 px-2 py-1 text-sm font-medium  text-purple-800'>{tech}</span>
          ))}
           </motion.div>
        </div>

    ))}
  </div>
    </div>
  )
}

export default Project
